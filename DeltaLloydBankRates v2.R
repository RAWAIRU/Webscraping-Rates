...
checkMonitor <- tryCatch(
  {
    ...
    source('functions/productRateDeltaLloydBank.R')
    source('settings/settingsDeltaLloydBank.R')
    ...
    for (productNum in 1:numberofProducts) {
      ...
      dfRates[productNum,4] <-  productRateDeltaLloydBank(tempUrl,tempNod,tempNod2,scaleFactor,outputFolder_pdf)
...
dbMonCol        <- c("Date", "Bank", "Rates stored?", "Time elapsed")
outMon          <- c(D,bank,checkMonitor, timeElapsed$toc - timeElapsed$tic)
monitor         <- storeEval(initiateMon,dbMonCol, outMon, dbMonitor,dbMonitorArc)
